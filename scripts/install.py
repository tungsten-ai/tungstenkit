...
# TODO Show warning if docker/nvidia-container-toolkit is not installed

r"""
This script will install Tungstenkit and its dependencies in an isolated fashion.

It will perform the following steps:
    * Create a new virtual environment using the built-in venv module, or the virtualenv zipapp if venv is unavailable.
      This will be created at a platform-specific path (or `$TUNGSTEN_HOME` if `$TUNGSTEN_HOME` is set:
        - `~/Library/Application Support/tungstenkit` on macOS
        - `$XDG_DATA_HOME/tungstenkit` on Linux/Unix (`$XDG_DATA_HOME` is `~/.local/share` if unset)
        - `%APPDATA%\tungstenkit` on Windows
    * Update pip inside the virtual environment to avoid bugs in older versions.
    * Install the latest (or a given) version of Tungstenkit inside this virtual environment using pip.
    * Install a `tungsten` script into a platform-specific path (or `$TUNGSTEN_HOME/bin` if `$TUNGSTEN_HOME` is set):
        - `~/.local/bin` on Unix
        - `%APPDATA%\Python\Scripts` on Windows
    * Attempt to inform the user if they need to add this bin directory to their `$PATH`, as well as how to do so.
    * Upon failure, write an error log to `tungstenkit-installer-error-<hash>.log and restore any previous environment.
...
Disclaimer:
This code is modified from https://github.com/python-poetry/install.python-poetry.org/blob/main/install-poetry.py.

Copyright (c) 2018 Sébastien Eustace
Copyright (c) 2023 Tungsten Contributors
"""  # noqa: E501
import sys

# Eager version check so we fail nicely before possible syntax errors
if sys.version_info < (3, 7):  # noqa: UP036
    sys.stdout.write("Tungstenkit installer requires Python 3.7 or newer to run!\n")
    sys.exit(1)
...
def data_dir() -> Path:
    if os.getenv("TUNGSTEN_HOME"):
        return Path(os.getenv("TUNGSTEN_HOME")).expanduser()
    ...
    return base_dir / "tungstenkit"


def bin_dir() -> Path:
    if os.getenv("TUNGSTEN_HOME"):
        return Path(os.getenv("TUNGSTEN_HOME")).expanduser() / "bin"
...
class TungstenkitInstallationError(RuntimeError):
...
class VirtualEnvironment:
    ...
    def make(cls, target: Path) -> "VirtualEnvironment":
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="tungsten-installer") as temp_dir:
                virtualenv_pyz = Path(temp_dir) / "virtualenv.pyz"
                request = Request(virtualenv_bootstrap_url, headers={"User-Agent": "Tungstenkit"})
                virtualenv_pyz.write_bytes(urlopen(request).read())
                cls.run(sys.executable, virtualenv_pyz, "--clear", "--always-copy", target)

        # We add a special file so that Tungstenkit can detect
        # its own virtual environment
        target.joinpath("tungsten_env").touch()
    ...
    def run(*args, **kwargs) -> subprocess.CompletedProcess:
        ...
        if completed_process.returncode != 0:
            raise TungstenkitInstallationError(
...
class Installer:
    METADATA_URL = "https://pypi.org/pypi/tungstenkit/json"
    VERSION_REGEX = re.compile(
        r"v?(\d+)(?:\.(\d+))?(?:\.(\d+))?(?:\.(\d+))?"
        "("
        "[._-]?"
        r"(?:(stable|beta|b|rc|RC|alpha|a|patch|post|pl|p)((?:[.-]?\d+)*)?)?"
        "([.-]?dev)?"
        ")?"
        r"(?:\+[^\s]+)?"
    )
    ...
    def install(self, version):
        ...
        with self.make_env(version) as env:
            self.install_tungstenkit(version, env)
    ...
    def make_bin(self, version: str, env: VirtualEnvironment) -> None:
        ...
        script = "tungsten.exe" if WINDOWS else "tungsten"
    ...
    def install_tungstenkit(self, version: str, env: VirtualEnvironment) -> None:
        ...
        if self._git:
            ...
            specification = f"tungstenkit=={version}"
    ...
    def get_version(self):
        ...
        if not version:
            for release in reversed(releases):
                ...
                if (
                    m.group(5) and not m.group(5).startswith(".post")
                ) and not self.allows_prereleases():
                    continue
    ...
    def _get(self, url):
        request = Request(url, headers={"User-Agent": "Python Tungstenkit"})
...
def main():
    parser = argparse.ArgumentParser(
        description="Installs the latest (or given) version of tungstenkit"
    )
    parser.add_argument(
        "-p",
        "--preview",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "fetching the latest version of Tungstenkit available online."
        ),
    )
    parser.add_argument(
        "--git",
        ...
        help=(
            ...
            "of Tungstenkit available online."
        ),
    )
    ...
    installer = Installer(
        version=args.version or os.getenv("TUNGSTENKIT_VERSION"),
        preview=args.preview or string_to_bool(os.getenv("TUNGSTENKIT_PREVIEW", "0")),
        force=args.force,
        accept_all=args.accept_all
        or string_to_bool(os.getenv("TUNGSTENKIT_ACCEPT", "0"))
        or not is_interactive(),
        path=args.path,
        git=args.git,
    )

    if args.uninstall or string_to_bool(os.getenv("TUNGSTENKIT_UNINSTALL", "0")):
        return installer.uninstall()

    try:
        return installer.run()
    except TungstenkitInstallationError as e:
        installer._write(colorize("error", "Tungstenkit installation failed."))
        ...
        if e.log is not None:
            ...
            _, path = tempfile.mkstemp(
                suffix=".log",
                prefix="tungstenkit-installer-error-",
                dir=str(Path.cwd()),
                text=True,
            )
